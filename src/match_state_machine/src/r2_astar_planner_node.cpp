...
public:
    R2AStarPlannerNode() : Node("r2_astar_planner_node") {
        ...
        declare_parameter<int>("type_unknown", -1);
        ...
        declare_parameter<double>("neighbor_tolerance", 0.05);
        ...
        declare_parameter<double>("cost_unknown_penalty", 1.0);

        declare_parameter<int>("target_count", 2);

        declare_parameter<bool>("auto_plan_when_update", true);
        declare_parameter<bool>("plan_when_r2_detected", true);
        declare_parameter<bool>("replan_when_fake_detected", true);
        declare_parameter<bool>("wait_all_blocks_before_plan", true);
        declare_parameter<bool>("plan_without_r2_target", true);
        declare_parameter<int>("default_explore_target_block", 10);
        declare_parameter<bool>("plan_once_on_start", true);
        declare_parameter<int>("initial_plan_delay_ms", 500);
        ...
        declare_parameter<std::string>("r2_type_prefix", "R2_KFS");
        declare_parameter<std::string>("fake_type_prefix", "FAKE_KFS");

        declare_parameter<double>("lost_confirm_time", 2.5);
...
declare_parameter<double>("locked_r1_refresh_radius_m", 0.55);
        ...
        */
        ...
        type_unknown_ = get_parameter("type_unknown").as_int();
        ...
        plan_when_r2_detected_ = get_parameter("plan_when_r2_detected").as_bool();
        replan_when_fake_detected_ = get_parameter("replan_when_fake_detected").as_bool();
        wait_all_blocks_before_plan_ = get_parameter("wait_all_blocks_before_plan").as_bool();
        plan_without_r2_target_ = get_parameter("plan_without_r2_target").as_bool();
        default_explore_target_block_ = get_parameter("default_explore_target_block").as_int();
        plan_once_on_start_ = get_parameter("plan_once_on_start").as_bool();
        initial_plan_delay_ms_ = get_parameter("initial_plan_delay_ms").as_int();
        ...
        r2_type_prefix_ = get_parameter("r2_type_prefix").as_string();
        fake_type_prefix_ = get_parameter("fake_type_prefix").as_string();
...
locked_r1_refresh_radius_m_ =
    get_parameter("locked_r1_refresh_radius_m").as_double();
        ...
        for (int i = 1; i <= 12; ++i) {
            block_types_[i] = type_unknown_;
        }
        block_types_[0] = type_empty_;
        block_types_[13] = type_empty_;
        block_types_[15] = type_empty_;
        ...
        kfs_sub_ = create_subscription<std_msgs::msg::String>(
            vision_kfs_topic_,
            10,
            std::bind(&R2AStarPlannerNode::vision_kfs_callback, this, std::placeholders::_1)
        );
        ...
        if (plan_once_on_start_) {
            initial_plan_timer_ = create_wall_timer(
                std::chrono::milliseconds(initial_plan_delay_ms_),
                [this]() {
                    if (initial_plan_timer_) {
                        initial_plan_timer_->cancel();
                    }

                    RCLCPP_WARN(
                        get_logger(),
                        "Initial planning triggered. plan_without_r2_target=%d, default_explore_target_block=%d",
                        plan_without_r2_target_ ? 1 : 0,
                        default_explore_target_block_
                    );
        ...
        RCLCPP_INFO(get_logger(), "Exit blocks: 13, 15");
...
private:
    ...
    void init_blocks() {
        blocks_[0]  = {0,  2.160, -1.530, 0.00};

        blocks_[1]  = {1,  3.360, -0.430, 0.40};
        blocks_[2]  = {2,  3.360, -1.630, 0.20};
        blocks_[3]  = {3,  3.360, -2.830, 0.40};

        blocks_[4]  = {4,  4.560, -0.430, 0.60};
        blocks_[5]  = {5,  4.560, -1.630, 0.40};
        blocks_[6]  = {6,  4.560, -2.830, 0.20};

        blocks_[7]  = {7,  5.760, -0.430, 0.40};
        blocks_[8]  = {8,  5.760, -1.630, 0.60};
        blocks_[9]  = {9,  5.760, -2.830, 0.40};

        blocks_[10] = {10, 6.960, -0.430, 0.20};
        blocks_[11] = {11, 6.960, -1.630, 0.40};
        blocks_[12] = {12, 6.960, -2.830, 0.20};

        // 出口下台阶点
        // 10 -> 13: 从 10 号下台阶出去
        // 12 -> 15: 从 12 号下台阶出去
        blocks_[13] = {13, 8.160, -0.430, 0.00};
        blocks_[15] = {15, 8.160, -2.830, 0.00};
    }
    ...
    void vision_kfs_callback(const std_msgs::msg::String::SharedPtr msg) {
        const std::string text = trim(msg->data);

        int detected_type = type_unknown_;

        if (starts_with(text, r1_type_prefix_)) {
            detected_type = type_r1_;
        } else if (starts_with(text, r2_type_prefix_)) {
            detected_type = type_r2_;
        } else if (starts_with(text, fake_type_prefix_)) {
            detected_type = type_fake_;
        } else {
            return;
        }
        ...
        if (!has_odom_) {
            RCLCPP_WARN_THROTTLE(
                ...
                *get_clock(),
                1000,
                "Received KFS but odom is unknown. Ignore. msg='%s'",
                text.c_str()
            );
            return;
        }
        ...
        if (!parse_kfs_camera_xyz(text, cam_x, cam_y, cam_z)) {
            RCLCPP_WARN(get_logger(), "Failed to parse KFS xyz from msg='%s'", text.c_str());
            return;
        }
        ...
        RCLCPP_WARN_THROTTLE(
            ...
            *get_clock(),
            300,
            "KFS projected result: text='%s', detected_type=%d, cam=(%.3f, %.3f, %.3f), "
            "field=(%.3f, %.3f), kfs_block=%d, current_block=%d, current_next_block=%d, "
            "look_backward=%d, robot=(%.3f, %.3f, yaw=%.3f)",
            text.c_str(), detected_type, cam_x, cam_y, cam_z,
            field_x, field_y, kfs_block, current_block_, current_next_block_,
            camera_look_backward_ ? 1 : 0, robot_x_, robot_y_, robot_yaw_rad_
        );
        ...
        if (kfs_block < 0) {
            RCLCPP_WARN_THROTTLE(
                ...
                *get_clock(),
                1000,
                "KFS out of blocks. cam=(%.3f, %.3f, %.3f), field=(%.3f, %.3f), robot=(%.3f, %.3f, %.3f)",
                cam_x, cam_y, cam_z, field_x, field_y, robot_x_, robot_y_, robot_yaw_rad_
            );
            return;
        }

        if (detected_type == type_r1_) {
            handle_r1_kfs_detection(kfs_block, field_x, field_y, cam_x, cam_y, cam_z);
        } else if (detected_type == type_r2_) {
            handle_r2_kfs_detection(kfs_block);
        } else if (detected_type == type_fake_) {
            handle_fake_kfs_detection(kfs_block);
        }
    }

    void handle_r2_kfs_detection(int kfs_block) {
        if (kfs_block < 1 || kfs_block > 12) {
            return;
        }

        block_types_[kfs_block] = type_r2_;
        received_blocks_.insert(kfs_block);
        detected_r2_targets_.insert(kfs_block);
        dynamic_passable_blocks_.erase(kfs_block);

        RCLCPP_WARN(
            get_logger(),
            "R2_KFS detected. block=%d, total_r2_targets=%ld",
            kfs_block,
            detected_r2_targets_.size()
        );

        if (plan_when_r2_detected_) {
            plan_and_publish();
        }
    }

    void handle_fake_kfs_detection(int kfs_block) {
        if (kfs_block < 1 || kfs_block > 12) {
            return;
        }

        block_types_[kfs_block] = type_fake_;
        received_blocks_.insert(kfs_block);
        detected_r2_targets_.erase(kfs_block);
        dynamic_passable_blocks_.erase(kfs_block);

        RCLCPP_WARN(
            get_logger(),
            "FAKE_KFS detected. block=%d marked as fake obstacle.",
            kfs_block
        );

        if (replan_when_fake_detected_) {
            plan_and_publish();
        }
    }

    void handle_r1_kfs_detection(
        int kfs_block,
        double field_x,
        double field_y,
        double cam_x,
        ...
        block_types_[kfs_block] = type_r1_;
        received_blocks_.insert(kfs_block);
        dynamic_passable_blocks_.erase(kfs_block);
        ...
        if (!is_next_block_r1_obstacle(kfs_block)) {
            if (r1_wait_state_ != R1WaitState::TRACKING_AND_WAITING) {
                RCLCPP_WARN_THROTTLE(
                    ...
                    *get_clock(),
                    1000,
                    "Ignore R1_KFS block=%d because current_next_block=%d. Only the next entering block is checked.",
                    kfs_block,
                    current_next_block_
                );
                return;
        ...
        if (r1_wait_state_ == R1WaitState::TRACKING_AND_WAITING) {
            ...
            const bool inside_locked_radius =
                is_detection_inside_locked_block_radius(locked_r1_block_, field_x, field_y, locked_dist);
            ...
            if (!inside_locked_radius) {
                ...
                RCLCPP_WARN_THROTTLE(
                    ...
                    *get_clock(),
                    500,
                    "Already waiting locked R1_KFS block=%d. Detection projected block=%d, field=(%.3f, %.3f), "
                    "locked_center=(%.3f, %.3f), dist=%.3f > radius=%.3f. Ignore and DO NOT refresh last_seen_time.",
                    locked_r1_block_, kfs_block, field_x, field_y, lb.x, lb.y,
                    locked_dist, locked_r1_refresh_radius_m_
                );
                return;
            }
            ...
            RCLCPP_INFO_THROTTLE(
                ...
                *get_clock(),
                500,
                "Locked R1_KFS block=%d refreshed by detection. projected_block=%d, field=(%.3f, %.3f), "
                "dist_to_locked=%.3f <= radius=%.3f.",
                locked_r1_block_, kfs_block, field_x, field_y, locked_dist, locked_r1_refresh_radius_m_
            );
        }
        ...
        if (r1_wait_state_ == R1WaitState::IDLE) {
            ...
            RCLCPP_WARN(
                ...
                current_block_, current_next_block_, locked_r1_block_
            );
        }
        ...
        RCLCPP_INFO_THROTTLE(
            ...
            *get_clock(),
            ...
            locked_r1_block_, current_block_, current_next_block_,
            cam_x, cam_y, cam_z, field_x, field_y, robot_x_, robot_y_, robot_yaw_rad_
        );
    }
    ...
    */
    ...
    void id_type_callback(const std_msgs::msg::UInt8MultiArray::SharedPtr msg) {
        ...
        */
        ...
        if (type == type_r1_) {
            ...
            detected_r2_targets_.erase(id);
        } else if (type == type_r2_) {
            detected_r2_targets_.insert(id);
            dynamic_passable_blocks_.erase(id);
        } else if (type == type_fake_) {
            detected_r2_targets_.erase(id);
            dynamic_passable_blocks_.erase(id);
        }
    ...
    void plan_and_publish() {
        ...
        for (int b : detected_r2_targets_) {
            if (b >= 1 && b <= 12 && block_types_[b] == type_r2_) {
                r2_blocks.push_back(b);
            }
        }

        const bool has_r2_target = !r2_blocks.empty();

        if (!has_r2_target) {
            if (!plan_without_r2_target_) {
                RCLCPP_WARN(
                    get_logger(),
                    "No detected R2 KFS blocks yet and plan_without_r2_target=false. Skip A* planning."
                );
                return;
            }

            int explore_target = choose_default_explore_target_block();

            if (explore_target < 0) {
                RCLCPP_ERROR(
                    get_logger(),
                    "No valid explore target block found. Skip A* planning."
                );
                return;
            }
            ...
            RCLCPP_WARN(
                get_logger(),
                "No R2_KFS detected yet. Plan exploration path to block=%d and then exit. "
                "No pickup_sequence will be generated.",
                explore_target
            );

            r2_blocks.push_back(explore_target);
        }
        ...
        if (!has_r2_target) {
            need_count = 1;
        } else if (static_cast<int>(r2_blocks.size()) < target_count_) {
            need_count = static_cast<int>(r2_blocks.size());
        }

        RCLCPP_INFO(get_logger(), has_r2_target ? "Detected R2 candidate blocks:" : "Explore candidate block:");
        {
            std::ostringstream oss;
        ...
        if (best.cost >= INF_ / 2.0) {
            RCLCPP_ERROR(get_logger(), "No feasible A* path found.");
            return;
        }
    ...
    int choose_default_explore_target_block() {
        std::vector<int> candidates;

        if (default_explore_target_block_ >= 1 && default_explore_target_block_ <= 12) {
            candidates.push_back(default_explore_target_block_);
        }

        for (int b : {10, 12, 11, 7, 9, 8, 4, 6, 5, 1, 2, 3}) {
        ...
        for (int b : candidates) {
            ...
            const double c = block_type_cost(b);
            if (c < INF_ / 2.0) {
                return b;
            }
            ...
            RCLCPP_WARN(
                get_logger(),
                "Explore candidate block=%d is not passable now, type_cost=%.3f. Try next.",
                b,
                c
            );
        }
    ...
    void evaluate_explore_order(const std::vector<int>& order, PlanResult& best) {
        active_targets_.clear();

        for (int exit_block : exit_blocks_) {
            PlanResult partial;
            partial.cost = 0.0;
            partial.targets.clear();
            partial.exit_block = exit_block;
            ...
            for (int explore_block : order) {
                SegmentResult best_segment;
                best_segment.cost = INF_;

                for (int yaw : yaws_) {
                    State goal_state{explore_block, yaw};
                    SegmentResult seg = astar(current, goal_state);

                    if (seg.cost < best_segment.cost) {
                        best_segment = seg;
                    }
                }
                ...
                append_segment(partial, best_segment);
                partial.cost += best_segment.cost;
                current = best_segment.states.back();
            }
    ...
    double heuristic_cost(const State& current, const State& goal) const {
        auto it_a = blocks_.find(current.block);
        auto it_b = blocks_.find(goal.block);

        if (it_a == blocks_.end() || it_b == blocks_.end()) {
            return 0.0;
        }
        ...
        double step = get_parameter("neighbor_distance").as_double();
        if (step <= 0.001) {
            step = 1.20;
        }
        ...
        return dist / step * min_move_cost;
    }
    ...
    SegmentResult astar(const State& start, const State& goal) {
        ...
        g_score[start] = 0.0;
        open.push({heuristic_cost(start, goal), 0.0, start});
        ...
        while (!open.empty()) {
            ...
            for (const auto& nb : neighbors) {
                ...
                if (tentative_g < g_score[next]) {
                    ...
                    const double f = tentative_g + heuristic_cost(next, goal);
                    open.push({f, tentative_g, next});
                }
            }
        }
    ...
    void get_neighbors(
        ...
        for (int nb : adjacency_[cur.block]) {
            ...
            if (opposite_yaw(cur.yaw) == move_dir) {
                if (std::fabs(dh) < 0.001) {
                    double c = get_parameter("cost_reverse_flat").as_double()
                             + height_cost
                             + type_cost;

                    out.push_back({
                        {nb, cur.yaw},
                        {"reverse_to_" + std::to_string(nb), c}
                    });
                } else if (std::fabs(dh + 0.20) < 0.001) {
    ...
    double block_type_cost(int block_id) {
        ...
        int type = type_unknown_;

        auto type_it = block_types_.find(block_id);
        if (type_it != block_types_.end()) {
            type = type_it->second;
        }

        if (type == type_unknown_) {
            return get_parameter("cost_unknown_penalty").as_double();
        }
...
private:
    ...
    std::set<int> detected_r2_targets_;
    ...
    bool plan_when_r2_detected_{true};
    bool replan_when_fake_detected_{true};
    bool wait_all_blocks_before_plan_;
    bool plan_without_r2_target_{true};
    int default_explore_target_block_{10};
    bool plan_once_on_start_{true};
    int initial_plan_delay_ms_{500};
    ...
    rclcpp::TimerBase::SharedPtr initial_plan_timer_;

    std::string odom_topic_{"/odom"};
    std::string vision_kfs_topic_{"/vision/kfs_camera_xyz"};
    std::string r1_type_prefix_{"R1_KFS"};
    std::string r2_type_prefix_{"R2_KFS"};
    std::string fake_type_prefix_{"FAKE_KFS"};
