...
class R2DijkstraPlannerNode : public rclcpp::Node {
public:
    R2DijkstraPlannerNode() : Node("r2_dijkstra_planner_node") {
        ...
        declare_parameter<int>("final_stop_block", 16);
        ...
        */
        ...
        final_stop_block_ = get_parameter("final_stop_block").as_int();
...
locked_r1_refresh_radius_m_ =
    get_parameter("locked_r1_refresh_radius_m").as_double();
        ...
        if (final_stop_block_ >= 0 && blocks_.find(final_stop_block_) == blocks_.end()) {
            RCLCPP_WARN(
                get_logger(),
                "Invalid final_stop_block=%d. Disable final stop append.",
                final_stop_block_);
            final_stop_block_ = -1;
        }
        ...
        id_type_sub_ = create_subscription<std_msgs::msg::UInt8MultiArray>(
            id_type_topic_,
            10,
            std::bind(&R2DijkstraPlannerNode::id_type_callback, this, std::placeholders::_1)
        );

        odom_sub_ = create_subscription<nav_msgs::msg::Odometry>(
            odom_topic_,
            10,
            std::bind(&R2DijkstraPlannerNode::odom_callback, this, std::placeholders::_1)
        );

        kfs_sub_ = create_subscription<std_msgs::msg::String>(
            vision_kfs_topic_,
            10,
            std::bind(&R2DijkstraPlannerNode::r1_kfs_callback, this, std::placeholders::_1)
        );
        ...
        camera_gimbal_state_sub_ = create_subscription<std_msgs::msg::Bool>(
            ...
            10,
            std::bind(
                &R2DijkstraPlannerNode::camera_gimbal_state_callback,
                this,
                std::placeholders::_1)
        );
        ...
        r1_kfs_monitor_timer_ = create_wall_timer(
            ...
            std::bind(&R2DijkstraPlannerNode::r1_kfs_monitor_timer_callback, this)
        );

        path_pub_ = create_publisher<std_msgs::msg::Int32MultiArray>("/r2_planned_path_raw", 10);
        target_pub_ = create_publisher<std_msgs::msg::Int32MultiArray>("/r2_target_kfs", 10);
        action_pub_ = create_publisher<std_msgs::msg::String>("/r2_action_sequence_raw", 10);
        ...
        RCLCPP_INFO(get_logger(), "R2 Dijkstra planner started.");
        ...
        RCLCPP_INFO(get_logger(), "Final stop block after exit: %d", final_stop_block_);
...
RCLCPP_INFO_THROTTLE(
    ...
    */
    ...
    void update_path_relevant_blocks(const PlanResult& plan) {
        ...
        append_final_stop_block(current_ordered_block_path_);
        if (final_stop_block_ >= 0 && blocks_.find(final_stop_block_) != blocks_.end()) {
            current_path_blocks_.insert(final_stop_block_);
            current_path_relevant_blocks_.insert(final_stop_block_);
        }
    ...
    void append_final_stop_block(std::vector<int>& block_path) const {
        if (final_stop_block_ < 0) {
            return;
        }

        if (blocks_.find(final_stop_block_) == blocks_.end()) {
            return;
        }

        if (!block_path.empty() && block_path.back() == final_stop_block_) {
            return;
        }

        block_path.push_back(final_stop_block_);
    }
        ...
        for (int exit_block : exit_blocks_) {
            ...
            for (int target_block : order) {
                ...
                for (const State& goal_state : pickup_states) {
                    SegmentResult seg = dijkstra(current, goal_state);
            ...
            for (int yaw : yaws_) {
                ...
                SegmentResult seg = dijkstra(current, exit_state);
    ...
    SegmentResult dijkstra(const State& start, const State& goal) {
        std::priority_queue<QueueItem, std::vector<QueueItem>, std::greater<QueueItem>> pq;

        std::unordered_map<State, double, StateKeyHash> dist;
        std::unordered_map<State, PrevInfo, StateKeyHash> prev;

        for (const auto& kv : blocks_) {
            for (int yaw : yaws_) {
                dist[{kv.first, yaw}] = INF_;
            }
        }
        ...
        while (!pq.empty()) {
            ...
            for (const auto& nb : neighbors) {
                ...
                double new_cost = dist[cur] + edge_cost;
    ...
    void publish_plan(const PlanResult& plan) {
        ...
        append_final_stop_block(block_path);
        ...
        if (final_stop_block_ >= 0) {
            oss << "(final_stop," << final_stop_block_ << ") ";
        }
        oss << "\n";
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<R2DijkstraPlannerNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
