...
public:
    ImuOdomBridge() : Node("imu_odom_bridge"), first_frame_(true) {
        ...
        this->declare_parameter<bool>("replace_zero_stamp_with_now", true);

        this->declare_parameter<double>("min_pose_xy_covariance", 1e-4);
        this->declare_parameter<double>("min_pose_yaw_covariance", 1e-4);
        this->declare_parameter<double>("fallback_pose_xy_covariance", 5e-3);
        this->declare_parameter<double>("fallback_pose_z_covariance", 1e3);
        this->declare_parameter<double>("fallback_pose_rp_covariance", 1e3);
        this->declare_parameter<double>("fallback_pose_yaw_covariance", 1e-2);
        ...
        replace_zero_stamp_with_now_ =
            this->get_parameter("replace_zero_stamp_with_now").as_bool();

        min_pose_xy_covariance_ =
            sanitizePositiveParameter("min_pose_xy_covariance", 1e-4);
        min_pose_yaw_covariance_ =
            sanitizePositiveParameter("min_pose_yaw_covariance", 1e-4);
        fallback_pose_xy_covariance_ =
            sanitizePositiveParameter("fallback_pose_xy_covariance", 5e-3);
        fallback_pose_z_covariance_ =
            sanitizePositiveParameter("fallback_pose_z_covariance", 1e3);
        fallback_pose_rp_covariance_ =
            sanitizePositiveParameter("fallback_pose_rp_covariance", 1e3);
        fallback_pose_yaw_covariance_ =
            sanitizePositiveParameter("fallback_pose_yaw_covariance", 1e-2);
...
private:
    ...
    double sanitizePositiveParameter(const std::string& name, double fallback) {
        double value = this->get_parameter(name).as_double();
        if (!std::isfinite(value) || value <= 0.0) {
            RCLCPP_WARN(
                this->get_logger(),
                "Parameter %s must be positive and finite. Use fallback %.6g.",
                name.c_str(),
                fallback
            );
            return fallback;
    ...
    bool finite3(double x, double y, double z) const {
        return std::isfinite(x) && std::isfinite(y) && std::isfinite(z);
    }

    bool inputFinite(const nav_msgs::msg::Odometry& msg) {
        const auto& p = msg.pose.pose.position;
        const auto& q = msg.pose.pose.orientation;
        const auto& lv = msg.twist.twist.linear;
        const auto& av = msg.twist.twist.angular;

        if (!finite3(p.x, p.y, p.z) || !finite3(q.x, q.y, q.z) ||
            !std::isfinite(q.w) || !finite3(lv.x, lv.y, lv.z) ||
            !finite3(av.x, av.y, av.z)) {
            RCLCPP_WARN_THROTTLE(
                this->get_logger(),
                *this->get_clock(),
                1000,
                "Drop invalid odometry: pose=(%.3f, %.3f, %.3f) quat=(%.3f, %.3f, %.3f, %.3f) "
    ...
    void sanitizePoseCovariance(nav_msgs::msg::Odometry& odom) const {
        auto& cov = odom.pose.covariance;
        bool bad_covariance = false;
        ...
        if (bad_covariance) {
            cov.fill(0.0);
            cov[0] = fallback_pose_xy_covariance_;
            cov[7] = fallback_pose_xy_covariance_;
            cov[14] = fallback_pose_z_covariance_;
            cov[21] = fallback_pose_rp_covariance_;
            cov[28] = fallback_pose_rp_covariance_;
            cov[35] = fallback_pose_yaw_covariance_;
            return;
        }
        ...
        if (cov[0] <= 0.0) {
            ...
            cov[0] = std::max(cov[0], min_pose_xy_covariance_);
        }

        if (cov[7] <= 0.0) {
            cov[7] = fallback_pose_xy_covariance_;
        } else {
            cov[7] = std::max(cov[7], min_pose_xy_covariance_);
        }

        if (cov[35] <= 0.0) {
            cov[35] = fallback_pose_yaw_covariance_;
        } else {
            cov[35] = std::max(cov[35], min_pose_yaw_covariance_);
        }
    ...
    void sanitizeTwistCovariance(nav_msgs::msg::Odometry& odom) const {
        auto& cov = odom.twist.covariance;
    ...
    void odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        if (replace_zero_stamp_with_now_ &&
            rclcpp::Time(odom_out.header.stamp).nanoseconds() == 0) {
        ...
        sanitizePoseCovariance(odom_out);
        ...
        sanitizeTwistCovariance(odom_out);
...
private:
    ...
    bool replace_zero_stamp_with_now_;

    double min_pose_xy_covariance_;
    double min_pose_yaw_covariance_;
    double fallback_pose_xy_covariance_;
    double fallback_pose_z_covariance_;
    double fallback_pose_rp_covariance_;
    double fallback_pose_yaw_covariance_;
